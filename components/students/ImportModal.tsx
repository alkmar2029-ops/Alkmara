...
export default function ImportModal({ grades, onClose, onDone }: ImportModalProps) {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="استيراد طلاب من Excel" maxWidth="max-w-2xl">
      {step === 1 && (<div className="space-y-4"><p className="text-sm text-gray-500">اختر طريقة الاستيراد:</p>
        <label className={`flex gap-3 p-4 rounded-lg border-2 cursor-pointer ${importType === 'specific' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}><input type="radio" checked={importType === 'specific'} onChange={() => setImportType('specific')} /><div><p className="font-medium">استيراد لشعبة محددة</p><p className="text-sm text-gray-500">حدد الصف والشعبة، ثم ارفع الملف</p></div></label>
        <label className={`flex gap-3 p-4 rounded-lg border-2 cursor-pointer ${importType === 'full' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}><input type="radio" checked={importType === 'full'} onChange={() => setImportType('full')} /><div><p className="font-medium">استيراد شامل</p><p className="text-sm text-gray-500">ارفع ملف يحتوي الصف والشعبة لكل طالب</p></div></label>
        <div className="flex justify-end"><button onClick={() => setStep(importType === 'specific' ? 2 : 3)} className="btn-primary">التالي</button></div></div>)}
      ...
      {step === 2 && (<div className="space-y-4">
        ...
        <div className="flex justify-between"><button onClick={() => setStep(1)} className="btn-secondary">السابق</button><button onClick={() => setStep(3)} disabled={!gradeId || !sectionId} className="btn-primary">التالي</button></div></div>)}

      {step === 3 && (<div className="space-y-4">
        <div className="border-2 border-dashed border-gray-300 rounded-xl p-12 text-center cursor-pointer hover:border-blue-400 transition-colors" onClick={() => fileInputRef.current?.click()}><FileSpreadsheet className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-600 font-medium">اسحب ملف Excel هنا أو اضغط للاختيار</p><p className="text-sm text-gray-400 mt-1">الصيغ: .xlsx, .xls, .csv</p></div>
        <input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} className="hidden" />
        <button onClick={downloadTemplate} className="text-blue-600 text-sm hover:underline flex items-center gap-1"><Download className="w-3 h-3" /> تحميل نموذج Excel</button>
        <div className="flex justify-between"><button onClick={() => setStep(importType === 'specific' ? 2 : 1)} className="btn-secondary">السابق</button></div></div>)}

      {step === 4 && (<div className="space-y-4">
        <div className="flex gap-4"><div className="flex items-center gap-2 text-green-600"><CheckCircle className="w-4 h-4" /> {validCount} صحيح</div><div className="flex items-center gap-2 text-red-600"><XCircle className="w-4 h-4" /> {errorCount} خطأ</div><div className="flex items-center gap-2 text-yellow-600"><AlertTriangle className="w-4 h-4" /> {duplicateCount} مكرر</div></div>
        <div className="max-h-80 overflow-y-auto border rounded-lg"><table className="w-full text-sm"><thead className="bg-gray-50 sticky top-0"><tr><th className="px-3 py-2 text-right">الحالة</th><th className="px-3 py-2 text-right">رقم الهوية</th><th className="px-3 py-2 text-right">الاسم</th><th className="px-3 py-2 text-right">الخطأ</th></tr></thead>
          <tbody>{validationResults.map((r, i) => (<tr key={i} className={`border-b ${r.status === 'error' ? 'bg-red-50' : r.status === 'duplicate' ? 'bg-yellow-50' : ''}`}><td className="px-3 py-2">{r.status === 'valid' && <CheckCircle className="w-4 h-4 text-green-500" />}{r.status === 'error' && <XCircle className="w-4 h-4 text-red-500" />}{r.status === 'duplicate' && <AlertTriangle className="w-4 h-4 text-yellow-500" />}</td><td className="px-3 py-2 font-mono">{r.student_id}</td><td className="px-3 py-2">{r.first_name} {r.father_name} {r.last_name}</td><td className="px-3 py-2 text-red-600 text-xs">{r.errors.join(', ')}</td></tr>))}</tbody></table></div>
        <div className="flex justify-between"><button onClick={() => setStep(3)} className="btn-secondary">السابق</button><button onClick={handleImport} disabled={importing || validCount === 0} className="btn-primary flex items-center gap-2"><Upload className="w-4 h-4" />{importing ? 'جاري الاستيراد...' : `استيراد ${validCount} طالب`}</button></div></div>)}

      {step === 5 && importResult && (<div className="space-y-4 text-center">
        <CheckCircle className="w-16 h-16 text-green-500 mx-auto" /><h3 className="text-xl font-bold">تم الاستيراد بنجاح</h3>
        <div className="flex gap-6 justify-center"><div><p className="text-2xl font-bold text-green-600">{importResult.imported}</p><p className="text-sm text-gray-500">تم استيرادهم</p></div><div><p className="text-2xl font-bold text-yellow-600">{importResult.skipped}</p><p className="text-sm text-gray-500">تم تخطيهم</p></div><div><p className="text-2xl font-bold text-red-600">{importResult.errors?.length || 0}</p><p className="text-sm text-gray-500">أخطاء</p></div></div>
        {importResult.errors?.length > 0 && (<div className="text-right bg-red-50 p-3 rounded-lg max-h-32 overflow-y-auto">{importResult.errors.map((e: string, i: number) => <p key={i} className="text-sm text-red-600">{e}</p>)}</div>)}
        <button onClick={onDone} className="btn-primary">إغلاق</button></div>)}
    </Modal>
  );
}
